    ...
    <div className="max-w-4xl space-y-4">
      ...
      {viewMode === 'board' && (() => {
        ...
        function BoardDroppable({ id, children, color }: { id: string; children: React.ReactNode; color: string }) {
          ...
          return (
            <div ref={setNodeRef} className={`space-y-2 min-h-[48px] rounded-lg transition-colors ${isOver ? 'ring-1 ring-inset' : ''}`} style={isOver ? { '--tw-ring-color': color } as React.CSSProperties : undefined}>
              {children}
            </div>
          );
        }
